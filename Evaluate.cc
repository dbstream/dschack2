...
namespace DSchack {
  /* These are the so called PeSTO tables, a very simple
     evaluation function which takes into consideration the
     placement of pieces on squares.

     The evaluation routine linearily interpolates between
     a middle-game score and an end-game score depending on
     the estimated game phase.  */
  ...
  void Accumulator::refresh(const Position &pos)
  {
	...
	m_score_mg += psqt_mg[piece][Sq(sqm) ^ 56];
	m_score_eg += psqt_eg[piece][Sq(sqm) ^ 56];
	m_phase += phaseInc[piece];
      }

      while (Bitboard sqm = PopLS1B(black_occ)) {
	m_score_mg -= psqt_mg[piece][Sq(sqm)];
	m_score_eg -= psqt_eg[piece][Sq(sqm)];
	m_phase += phaseInc[piece];
      }
    }
  }
  ...
  void Accumulator::addPiece(Color color, PieceType piece, int sq)
  {
    m_phase += phaseInc[piece];
    if (color == WHITE) {
      m_score_mg += psqt_mg[piece][sq ^ 56];
      m_score_eg += psqt_eg[piece][sq ^ 56];
    } else {
      m_score_mg -= psqt_mg[piece][sq];
      m_score_eg -= psqt_eg[piece][sq];
    }
  }
